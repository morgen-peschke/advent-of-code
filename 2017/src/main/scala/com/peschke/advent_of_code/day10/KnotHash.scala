...
/**
  ...
  * programs are implementing isn't very strong, but you decide to
  * help them anyway. You make a mental note to remind the Elves later
  * not to invent their own cryptographic functions.
  *
  ...
  * current position which begins at 0 (the first element in the
  * list), a skip size (which starts at 0), and a sequence of lengths
  ...
  * of the list. If the current position moves past the end of the
  * list, it wraps around to the front. Lengths larger than the size
  * of the list are invalid.
  *
  ...
  * (with values 0 to 255) and the sequence of lengths in your puzzle
  ...
  * length sequence (3, 4, 1, 5, 17, 31, 73, 47, 23, now assuming they
  * came from ASCII codes and include the suffix), but start with the
  ...
  * XOR'd together, the second element in the dense hash is the second
  ...
  * six characters of the hash would be 4007ff. Because every Knot
  * Hash is sixteen such numbers, the hexadecimal representation is
...
object KnotHash extends AdventOfCodeDay[Int, RenderedHash] {
  ...
  def verifyPart1Samples(): Unit = {
    import Part1._
    println("-- Wrapping --")
    val circularBuffer = CircularBuffer.containing('a', 'b', 'c', 'd', 'e')
    val accessElement = circularBuffer.liftedToTry
    Seq(
      ( 0, 'a'),
      ( 1, 'b'),
      ( 2, 'c'),
      ...
      ( 9, 'e'),
      (10, 'a'))
      .map((verifyResult(accessElement) _).tupled)
      .map("   " + _)
      .foreach(println)

    println("-- Worked Example --")
    def next(length: Int, initial: HashContext[Int], expected: HashContext[Int]) =
      "   " + verifyResult((initial.next(_)).liftedToTry)(length, expected)
    Seq(
      (
        3,
        HashContext(CircularBuffer.containing(0, 1, 2, 3, 4), 0, 0),
        HashContext(CircularBuffer.containing(2, 1, 0, 3, 4), 3, 1)
      ),
      (
        4,
        HashContext(CircularBuffer.containing(2, 1, 0, 3, 4), 3, 1),
        HashContext(CircularBuffer.containing(4, 3, 0, 1, 2), 3, 2),
      ),
      (
        1,
        HashContext(CircularBuffer.containing(4, 3, 0, 1, 2), 3, 2),
        HashContext(CircularBuffer.containing(4, 3, 0, 1, 2), 1, 3),
      ),
      (
        5,
        HashContext(CircularBuffer.containing(4, 3, 0, 1, 2), 1, 3),
        HashContext(CircularBuffer.containing(3, 4, 2, 1, 0), 4, 4),
      )
    ).map((next _).tupled).foreach(println)
  }
