...
/**
  ...
  * defragmenter. Were the situation different, you might sit and
  * watch it for a while, but today, you just don't have that kind of
  ...
  * soon as possible.
  *
  * The disk in question consists of a 128x128 grid; each square of
  * the grid is either free or used. On this disk, the state of the
  * grid is tracked by the bits in a sequence of knot hashes.
  *
  ...
  * correspond to individual grid squares. Each bit of a hash
  ...
  * by the bits of the knot hash of flqrgnkx-0, the second row from
  * the bits of the knot hash of flqrgnkx-1, and so on until the last
  * row, flqrgnkx-127.
  *
  ...
  * 0 becomes 0000, 1 becomes 0001, e becomes 1110, f becomes 1111,
  * and so on; a hash that begins with a0c2017... in hexadecimal would
  ...
  * --- Part Two ---
  *
  * Now, all the defragmenter needs to know is the number of
  * regions. A region is a group of used squares that are all
  * adjacent, not including diagonals. Every used square is in exactly
  * one region: lone used squares form their own isolated regions,
