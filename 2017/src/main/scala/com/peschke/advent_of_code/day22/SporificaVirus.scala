...
/**
  ...
  *
  * Diagnostics indicate that the local grid computing cluster has
  * been contaminated with the Sporifica Virus. The grid computing
  * cluster is a seemingly-infinite two-dimensional grid of compute
  * nodes. Each node is either clean or infected by the virus.
  *
  * To prevent overloading the nodes (which would render them useless
  * to the virus) or detection by system administrators, exactly one
  * virus carrier moves through the network, infecting or cleaning
  * nodes as it moves. The virus carrier is always located on a single
  * node in the network (the current node) and keeps track of the
  ...
  *   - The virus carrier moves forward one node in the direction it
  * is facing.
  *
  ...
  * }}}
  *
  * By this time, 41 bursts of activity caused an infection (though
  * most of those nodes have since been cleaned).
  *
  ...
  * --- Part Two ---
  *
  * As you go to remove the virus from the infected nodes, it evolves
  * to resist your attempt.
  *
  ...
  *   - Flagged nodes become cleanq.
  *
  * Every node is always in exactly one of the above states.
  *
  * The virus carrier still functions in a similar way, but now uses
  ...
  * looks like this, with the virus carrier's position again marked
  * with [ ]:
  *
  * {{{
  * . . . . . . . . .
  ...
  * begin infected.)
  */
object SporificaVirus extends AdventOfCodeDay {
  type P1 = Int
  type P2 = Int

  def runPart1(input: String): Try[P1] =
    Part1.simulate(input, 10000).map(_.burstsCausingInfections)

  def runPart2(input: String): Try[P2] =
    Part2.simulate(input, 10000000).map(_.burstsCausingInfections)
        ...
        state = State(
          Grid.parseOrDie(
            """...##..
              ...
              |...##..""".stripMargin),
          Facing.North at(-1, 1)),
        burstsCausingInfections = 41)
    ).map((verifyResult(Part1.simulate(input, _: Int)) _).tupled).foreach(println)

    println(
      Part1
        .simulate(input, 10000)
        .map(_.burstsCausingInfections)
        .asResult(5587)
        .show)
  }
        ...
        state = State(
          Grid.parseOrDie(
            """WW.#
              |#.W.""".stripMargin),
          Facing.West at(0, -3)),
        burstsCausingInfections = 1)
    ).map((verifyResult(Part2.simulate(input, _: Int)) _).tupled).foreach(println)
