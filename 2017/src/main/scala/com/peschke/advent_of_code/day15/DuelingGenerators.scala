...
/**
  ...
  * sequence of numbers. However, one of them is malfunctioning, and
  ...
  * 65, while generator B uses 8921. Then, the first five pairs of
  ...
  * value before comparing them (using the same comparison method as
  * before). It keeps track of the order it receives values; the first
  * values from each generator are compared, then the second values
  * from each generator, then the third values, and so on.
  *
  ...
  */
object DuelingGenerators extends AdventOfCodeDay {
  type P1 = BigInt
  type P2 = BigInt

  def runPart1(input: String): Try[P1] = Part1.passJudgement(input)
  def runPart2(input: String): Try[P2] = Part2.passJudgement(input)

  implicit val bigIntEq: Eq[BigInt] = Eq.fromUniversalEquals[BigInt]
  implicit val bigIntShow: Show[BigInt] = Show.fromToString[BigInt]

  implicit val generatorEq: Eq[Generator] = Eq.fromUniversalEquals[Generator]
  implicit val generatorShow: Show[Generator] = cats.derive.show[Generator]

  implicit val generatorWithFilterEq: Eq[GeneratorWithFilter] = cats.derive.eq[GeneratorWithFilter]
  implicit val generatorWithFilterShow: Show[GeneratorWithFilter] = cats.derive.show[GeneratorWithFilter]

  private val sampleInput = """|Generator A starts with 65
  ...
  def verifyPart1Samples(): Unit = {
    ...
    val genA = Generator("A", BigInt(65), BigInt(16807))
    val genAValues =
      Seq(
        BigInt(1092455),
        BigInt(1181022009),
    ...
    val genB = Generator("B", BigInt(8921), BigInt(48271))
    val genBValues =
      Seq(
        BigInt(430625591),
        BigInt(1233683848),
    ...
    println("--- parsing ---")
    println(verifyResult(parse)(sampleInput, Vector(genA, genB)))
    ...
    genA.generate
      ...
      .toSeq
      .zip(genAValues)
      .map((verifyResult((identity[BigInt] _).liftedToTry) _).tupled)
      .zipWithIndex
      .map(p => s"${p._2}: ${p._1}")
      .foreach(println)

    println("\n--- values generated by B ---")
    genB.generate
      .take(5)
      .toSeq
      .zip(genBValues)
    ...
    println("\n--- judging ---")
    println(verifyResult(passJudgement)(sampleInput, BigInt(588)))
  }

  def verifyPart2Samples(): Unit = {
    import Part2._

    val genA = GeneratorWithFilter(Generator("A", BigInt(65), BigInt(16807)), BigInt(4))
    val genAValues =
      Seq(
        BigInt(1352636452),
        BigInt(1992081072),
    ...
    val genB = GeneratorWithFilter(Generator("B", BigInt(8921), BigInt(48271)), BigInt(8))
    val genBValues =
      Seq(
        BigInt(1233683848),
        BigInt(862516352),
    ...
    println("\n--- judging ---")
    println(verifyResult(passJudgement)(sampleInput, BigInt(309)))
  }
}
